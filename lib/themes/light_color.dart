import 'package:flutter/material.dart';

class LightColor {
  static const Color background = Color(0XFFFFFFFF);

  static const Color titleTextColor = const Color(0xff1d2635);
  static const Color subTitleTextColor = const Color(0xff797878);

  static const Color skyBlue = Color(0xff2890c8);
  static const Color lightBlue = Color(0xff5c3dff);
  static const Color lightWhite = Color(0xffffffff);
  static const Color lightBabyBlue = Color(0xff2792b4);
  static const Color blue = Color(0xff1959a9);
  static const Color facebookBlue = Color(0xff2872ba);

  static const Color orange = Color(0xffE65829);
  static const Color red = Color(0xffF72804);

  static const Color lightGrey = Color(0xffE1E2E4);
  static const Color moreLightGrey = Color(0xfff8f8f8);
  static const Color grey = Color(0xffA1A3A6);
  static const Color darkgrey = Color(0xff747F8F);

  static const Color iconColor = Color(0xffa8a09b);
  static const Color yellowColor = Color(0xfffbba01);

  static const Color black = Color(0xff20262C);
  static const Color lightblack = Color(0xff5F5F60);

  static const Color lightorange = Color(0xfff7892b);
  // static const Color lightgreen = Color(0xff6bbc14);
  static const Color lightDarkGreen = Color(0xFF305C02);
  static const Color secondaryColor = Color(0xffA1A3A6);

  static const Color brandFirstColor = Colors.black;
  static const Color brandSecondColor = Color(0xFF558621);

  static blackHintText() => TextStyle(
        color: Colors.black,
        fontSize: 16,
      );
}
